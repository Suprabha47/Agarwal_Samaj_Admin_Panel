...
export default function Articles() {
  ...
  const filterData =
    Cat === "All"
      ? Articles
      : Articles.filter((item) => item.Category.category_name === Cat);
            ...
            onClick={async () => {
              try {
                const response = await axios.delete(
                  `${process.env.REACT_APP_BACKEND_URL}/api/blogs/${post_id}`
                );
                if (response?.data) {
      ...
      <main className="flex-1 flex flex-col md:ml-64">
        ...
        <div className="mb-6 mt-5 ">
          ...
          <p className="text-gray-600 text-lg ml-5">
            Manage all your blog posts and news articles
          </p>
        </div>
                    ...
                    <td className="px-6 py-3 text-center space-x-2">
                      <div className="inline-flex gap-3">
                        <button className="px-3 py-1 text-white bg-blue-600 hover:bg-blue-700 rounded-md text-lg">
                          <NavLink to={`article/${item.post_id}`}>View</NavLink>
                        </button>
                        <button
